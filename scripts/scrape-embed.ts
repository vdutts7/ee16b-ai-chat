...
async function extractDataFromUrls(urls: string[]): Promise<Document[]> {
  console.log('extracting text from the EE16B website...');
  const documents: Document[] = [];
  for (const url of urls) {
    const docs = await extractDataFromUrl(url);
    documents.push(...docs);
  }
  console.log('data extracted from the EE16B website');
  const json = JSON.stringify(documents);
  await fs.writeFile('ee16b.json', json);
  console.log('json file containing text from the EE16B class website ~ saved on disk');
  return documents;
}
